...
@router.get('/all', response_model=list[TankResponse], name="Получить список всех резервуаров")
...
@router.get('/get/{tank_id}/new_current_capacity', response_model=TankResponse, name="Обновить значение резервуара")
def get(tank_id: int,
    ...
    if get_with_check(tank_id, tanks_service):
        return tanks_service.update_capacity(tank_id, new_current_capacity)
